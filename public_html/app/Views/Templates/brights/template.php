<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <title>Home2</title>
    <meta name="description" content="#">
    <link rel="stylesheet" href="app/Views/Templates/brights/dist/css/grayshift.min.css">
    <link rel="stylesheet" href="app/Views/Templates/brights/dist/css/brights.min.css">
    <link rel="stylesheet" href="https://rsms.me/inter/inter.css" integrity="sha384-bkV/AssUyv83vA3BCj2g2Nn7rv1tW1gkXC1CPrFj0v9QP2WDMho6tbf18S99RNeT" crossorigin="anonymous">
</head>
<body>
<nav class="navbar navbar-expand-lg nav-light bg-navy">
    <div class="container">
        <a rel="home" href="#">
            <svg class="gi gi-github text-white" width="88.73" height="24" viewBox="-0.122 175.062 480.244 129.875" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <path d="M92.375 230.65H52.019a1.887 1.887 0 0 0-1.886 1.886v19.731a1.89 1.89 0 0 0 1.886 1.889h15.743v24.513s-3.535 1.206-13.308 1.206c-11.53 0-27.637-4.214-27.637-39.632 0-35.425 16.772-40.087 32.518-40.087 13.63 0 19.502 2.401 23.238 3.556 1.174.36 2.26-.808 2.26-1.851l4.502-19.063c0-.487-.165-1.075-.721-1.474-1.517-1.082-10.774-6.262-34.16-6.262-26.941 0-54.576 11.463-54.576 66.562 0 55.103 31.64 63.313 58.302 63.313 22.076 0 35.468-9.434 35.468-9.434.552-.306.612-1.077.612-1.431v-61.537c0-1.04-.844-1.885-1.885-1.885M300.354 181.664a1.88 1.88 0 0 0-1.873-1.896h-22.724a1.889 1.889 0 0 0-1.882 1.896l.006 43.913h-35.419v-43.913a1.882 1.882 0 0 0-1.875-1.896h-22.722a1.887 1.887 0 0 0-1.877 1.896v118.904c0 1.048.843 1.901 1.877 1.901h22.722c1.039 0 1.875-.854 1.875-1.901v-50.86h35.419l-.062 50.86c0 1.048.844 1.901 1.883 1.901h22.777c1.041 0 1.873-.854 1.875-1.901V181.664M135.254 197.268c0-8.183-6.56-14.795-14.653-14.795-8.085 0-14.65 6.612-14.65 14.795 0 8.173 6.565 14.802 14.65 14.802 8.093 0 14.653-6.629 14.653-14.802M133.629 275.486V220.6a1.89 1.89 0 0 0-1.88-1.895h-22.651c-1.039 0-1.969 1.072-1.969 2.115v78.634c0 2.312 1.44 2.998 3.304 2.998h20.408c2.239 0 2.788-1.1 2.788-3.034v-23.932M386.71 218.884h-22.549c-1.034 0-1.876.852-1.876 1.901v58.302s-5.729 4.192-13.859 4.192c-8.13 0-10.287-3.689-10.287-11.65v-50.844c0-1.049-.841-1.901-1.875-1.901h-22.886c-1.032 0-1.879.852-1.879 1.901v54.693c0 23.646 13.179 29.431 31.31 29.431 14.873 0 26.864-8.216 26.864-8.216s.571 4.329.829 4.843c.259.512.932 1.029 1.659 1.029l14.56-.064c1.032 0 1.879-.854 1.879-1.898l-.008-79.817c0-1.05-.843-1.902-1.882-1.902M439.445 283.207c-7.821-.238-13.126-3.788-13.126-3.788v-37.653s5.233-3.208 11.654-3.782c8.12-.727 15.944 1.726 15.944 21.095.001 20.426-3.53 24.457-14.472 24.128zm8.894-66.993c-12.807 0-21.518 5.714-21.518 5.714v-40.263c0-1.05-.839-1.897-1.875-1.897h-22.787a1.887 1.887 0 0 0-1.878 1.897v118.903c0 1.048.842 1.901 1.881 1.901h15.811c.711 0 1.25-.367 1.648-1.009.393-.639.96-5.482.96-5.482s9.318 8.83 26.957 8.83c20.708 0 32.584-10.504 32.584-47.154 0-36.651-18.967-41.44-31.783-41.44M198.936 218.695h-17.045l-.026-22.519c0-.852-.439-1.278-1.424-1.278h-23.228c-.903 0-1.388.398-1.388 1.265v23.271s-11.64 2.809-12.427 3.037a1.886 1.886 0 0 0-1.361 1.812v14.623c0 1.052.84 1.898 1.879 1.898h11.909v35.178c0 26.13 18.328 28.697 30.696 28.697 5.651 0 12.411-1.814 13.527-2.228.675-.248 1.067-.947 1.067-1.705l.019-16.086c0-1.05-.886-1.897-1.884-1.897-.993 0-3.534.404-6.15.404-8.372 0-11.209-3.893-11.209-8.932l-.001-33.432h17.046c1.039 0 1.88-.847 1.88-1.898v-18.317a1.884 1.884 0 0 0-1.88-1.893"/>
            </svg>
        </a>
        <button class="navbar-toggler" data-toggle="collapse" data-target="#navbarSupportedContent" type="button" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle Navigation">
            <svg class="gi gi-menu fs-lg text-white" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                <rect x="3" y="11" width="18" height="2" rx=".95" ry=".95"/>
                <rect x="3" y="16" width="18" height="2" rx=".95" ry=".95"/>
                <rect x="3" y="6" width="18" height="2" rx=".95" ry=".95"/>
            </svg>
        </button>
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav">
                <li class="nav-item dropdown">
                    <a class="nav-link" id="navbarDropdownOne" data-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">Hosting<svg class="gi gi-arrow-down-fill ml-1 align-text-top" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 17a1.72 1.72 0 0 1-1.33-.64l-4.21-5.1a2.1 2.1 0 0 1-.26-2.21A1.76 1.76 0 0 1 7.79 8h8.42a1.76 1.76 0 0 1 1.59 1.05 2.1 2.1 0 0 1-.26 2.21l-4.21 5.1A1.72 1.72 0 0 1 12 17z"/>
                        </svg></a>
                    <div class="dropdown-menu" aria-labelledby="navbarDropdownOne">
                        <a class="dropdown-item" href="#">
                <span class="icon mr-4 bg-primary rounded-circle">
                  <svg class="gi gi-star-fill fs-sm text-white" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path d="M17.56 21a1 1 0 0 1-.46-.11L12 18.22l-5.1 2.67a1 1 0 0 1-1.45-1.06l1-5.63-4.12-4a1 1 0 0 1-.25-1 1 1 0 0 1 .81-.68l5.7-.83 2.51-5.13a1 1 0 0 1 1.8 0l2.54 5.12 5.7.83a1 1 0 0 1 .81.68 1 1 0 0 1-.25 1l-4.12 4 1 5.63a1 1 0 0 1-.4 1 1 1 0 0 1-.62.18z"/>
                  </svg>
                </span>
                            <div>
                                <h2 class="fs-xxs lh-lg">Droplets</h2>
                                <p>Scalable compute services</p>
                            </div>
                        </a>
                        <a class="dropdown-item" href="#">
                <span class="icon mr-4 bg-success rounded-circle">
                  <svg class="gi gi-shield-fill fs-sm text-white" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 21.85a2 2 0 0 1-1-.25l-.3-.17A15.17 15.17 0 0 1 3 8.23v-.14a2 2 0 0 1 1-1.75l7-3.94a2 2 0 0 1 2 0l7 3.94a2 2 0 0 1 1 1.75v.14a15.17 15.17 0 0 1-7.72 13.2l-.3.17a2 2 0 0 1-.98.25z"/>
                  </svg>
                </span>
                            <div>
                                <h2 class="fs-xxs lh-lg">Spaces</h2>
                                <p>Simple object storage</p>
                            </div>
                        </a>
                        <a class="dropdown-item" href="#">
                <span class="icon mr-4 bg-info rounded-circle">
                  <svg class="gi gi-settings-fill fs-sm text-white" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <circle cx="12" cy="12" r="1.5"/>
                    <path d="M21.89 10.32L21.1 7.8a2.26 2.26 0 0 0-2.88-1.51l-.34.11a1.74 1.74 0 0 1-1.59-.26l-.11-.08a1.76 1.76 0 0 1-.69-1.43v-.28a2.37 2.37 0 0 0-.68-1.68 2.26 2.26 0 0 0-1.6-.67h-2.55a2.32 2.32 0 0 0-2.29 2.33v.24a1.94 1.94 0 0 1-.73 1.51l-.13.1a1.93 1.93 0 0 1-1.78.29 2.14 2.14 0 0 0-1.68.12 2.18 2.18 0 0 0-1.12 1.33l-.82 2.6a2.34 2.34 0 0 0 1.48 2.94h.16a1.83 1.83 0 0 1 1.12 1.22l.06.16a2.06 2.06 0 0 1-.23 1.86 2.37 2.37 0 0 0 .49 3.3l2.07 1.57a2.25 2.25 0 0 0 1.35.43A2 2 0 0 0 9 22a2.25 2.25 0 0 0 1.47-1l.23-.33a1.8 1.8 0 0 1 1.43-.77 1.75 1.75 0 0 1 1.5.78l.12.17a2.24 2.24 0 0 0 3.22.53L19 19.86a2.38 2.38 0 0 0 .5-3.23l-.26-.38A2 2 0 0 1 19 14.6a1.89 1.89 0 0 1 1.21-1.28l.2-.07a2.36 2.36 0 0 0 1.48-2.93zM12 15.5a3.5 3.5 0 1 1 3.5-3.5 3.5 3.5 0 0 1-3.5 3.5z"/>
                  </svg>
                </span>
                            <div>
                                <h2 class="fs-xxs lh-lg">Block Storage</h2>
                                <p>Connect, share and learn</p>
                            </div>
                        </a>
                    </div>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#">Мои проэкты</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#">Ресурсы</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" href="#">Company</a>
                </li>
            </ul>
            <div class="d-flex">
                <a class="btn btn-sm btn-primary-light w-sm-100 mr-3" href="#">Login</a>
                <a class="btn btn-sm btn-info w-sm-100" href="#">Sign Up</a>
            </div>
        </div>
    </div>
</nav>
<div class="py-11 bg-navy">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg">
                <div class="text-center text-lg-left">
                    <h1 class="mb-5 fs-lg font-weight-extra-bold text-white">Портфолио</h1>
                    <p class="mb-5 lead text-muted">Просто не значит плохо, сайт работает под любой браузер, и любое устройство.</p>
                    <a class="btn btn-lg btn-primary-light w-sm-100 px-sm-8" href="#">Get Started</a>
                </div>
            </div>
            <div class="col-lg">
                <img class="img-fluid d-none d-lg-inline float-right" src="app/Views/Templates/brights/dist/img/illustration.png" alt="illustration">
            </div>
        </div>
    </div>
</div>
<div class="card rounded-0">
    <div class="container">
        <ul class="list-unstyled d-flex justify-content-between py-6 overflow-x-auto overflow-y-hidden scrollbar-none">
            <li class="mr-9">
                <svg class="gi gi-amazon" width="100.92" height="100%" viewBox="163.5 263.1 285 85.8" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" d="m328.2 272.5v-6.5c0-1 .7-1.6 1.6-1.6h29c.9 0 1.7.7 1.7 1.6v5.5c0 .9-.8 2.1-2.2 4.1l-15 21.4c5.6-.1 11.5.7 16.5 3.5 1.1.6 1.4 1.6 1.5 2.5v6.9c0 1-1 2.1-2.1 1.5-8.9-4.7-20.8-5.2-30.6.1-1 .5-2.1-.5-2.1-1.5v-6.6c0-1 0-2.8 1.1-4.4l17.4-24.9h-15.1c-.9 0-1.7-.7-1.7-1.6zm-105.7 40.3h-8.8c-.8-.1-1.5-.7-1.6-1.5v-45.2c0-.9.8-1.6 1.7-1.6h8.2c.9 0 1.5.7 1.6 1.5v5.9h.2c2.1-5.7 6.2-8.4 11.6-8.4 5.5 0 9 2.7 11.4 8.4 2.1-5.7 7-8.4 12.2-8.4 3.7 0 7.7 1.5 10.2 5 2.8 3.8 2.2 9.3 2.2 14.2v28.6c0 .9-.8 1.6-1.7 1.6h-8.7c-.9-.1-1.6-.8-1.6-1.6v-24c0-1.9.2-6.7-.2-8.5-.7-3-2.6-3.9-5.2-3.9-2.1 0-4.4 1.4-5.3 3.7s-.8 6.1-.8 8.7v24c0 .9-.8 1.6-1.7 1.6h-8.8c-.9-.1-1.6-.8-1.6-1.6v-24c0-5 .8-12.5-5.4-12.5-6.3 0-6.1 7.2-6.1 12.5v24c-.1.8-.8 1.5-1.8 1.5zm163-49.3c13.1 0 20.2 11.2 20.2 25.5 0 13.8-7.8 24.8-20.2 24.8-12.8 0-19.8-11.2-19.8-25.2-.1-14.1 7-25.1 19.8-25.1zm0 9.3c-6.5 0-6.9 8.9-6.9 14.4s-.1 17.3 6.8 17.3c6.8 0 7.2-9.5 7.2-15.3 0-3.8-.2-8.4-1.3-12-1-3.2-3-4.4-5.8-4.4zm37.1 40h-8.8c-.9-.1-1.6-.8-1.6-1.6v-45.3c.1-.8.8-1.5 1.7-1.5h8.2c.8 0 1.4.6 1.6 1.3v6.9h.2c2.5-6.2 5.9-9.1 12-9.1 3.9 0 7.8 1.4 10.3 5.3 2.3 3.6 2.3 9.7 2.3 14.1v28.5c-.1.8-.8 1.4-1.7 1.4h-8.8c-.8-.1-1.5-.7-1.6-1.4v-24.6c0-5 .6-12.2-5.5-12.2-2.1 0-4.1 1.4-5.1 3.6-1.2 2.8-1.4 5.5-1.4 8.6v24.4c-.1.9-.9 1.6-1.8 1.6zm-117.5-21.6c0 3.4.1 6.3-1.6 9.4-1.4 2.5-3.6 4-6.1 4-3.4 0-5.4-2.6-5.4-6.4 0-7.5 6.7-8.9 13.1-8.9zm8.9 21.5c-.6.5-1.4.6-2.1.2-2.9-2.4-3.5-3.6-5.1-5.9-4.8 4.9-8.3 6.4-14.5 6.4-7.4 0-13.2-4.6-13.2-13.7 0-7.2 3.9-12 9.4-14.4 4.8-2.1 11.5-2.5 16.6-3.1v-1.1c0-2.1.2-4.6-1.1-6.4-1.1-1.6-3.1-2.3-4.9-2.3-3.4 0-6.4 1.7-7.1 5.3-.2.8-.7 1.6-1.5 1.6l-8.5-.9c-.7-.2-1.5-.7-1.3-1.8 2-10.4 11.3-13.5 19.7-13.5 4.3 0 9.9 1.1 13.3 4.4 4.3 4 3.9 9.4 3.9 15.2v13.7c0 4.1 1.7 5.9 3.3 8.2.6.8.7 1.8 0 2.3-1.9 1.5-5.1 4.3-6.9 5.8zm-124.4-21.5c0 3.4.1 6.3-1.6 9.4-1.4 2.5-3.6 4-6.1 4-3.4 0-5.4-2.6-5.4-6.4 0-7.5 6.7-8.9 13.1-8.9zm8.8 21.5c-.6.5-1.4.6-2.1.2-2.9-2.4-3.5-3.6-5.1-5.9-4.8 4.9-8.3 6.4-14.5 6.4-7.4 0-13.2-4.6-13.2-13.7 0-7.2 3.9-12 9.4-14.4 4.8-2.1 11.5-2.5 16.6-3.1v-1.1c0-2.1.2-4.6-1.1-6.4-1.1-1.6-3.1-2.3-4.9-2.3-3.4 0-6.4 1.7-7.1 5.3-.2.8-.7 1.6-1.5 1.6l-8.5-.9c-.7-.2-1.5-.7-1.3-1.8 2-10.4 11.3-13.5 19.7-13.5 4.3 0 9.9 1.1 13.3 4.4 4.3 4 3.9 9.4 3.9 15.2v13.7c0 4.1 1.7 5.9 3.3 8.2.6.8.7 1.8 0 2.3-1.9 1.5-5.1 4.3-6.9 5.8z" clip-rule="evenodd"/>
                    <path d="m340.3 330.2c-16.5 12.2-40.5 18.7-61.2 18.7-29 0-55-10.7-74.8-28.5-1.5-1.4-.2-3.3 1.7-2.2 21.3 12.4 47.6 19.8 74.8 19.8 18.3 0 38.5-3.8 57.1-11.7 2.8-1.1 5.1 1.9 2.4 3.9z"/>
                    <path d="m347.2 322.3c-2.1-2.7-14-1.3-19.3-.6-1.6.2-1.9-1.2-.4-2.2 9.5-6.7 25-4.7 26.8-2.5s-.5 17.8-9.4 25.2c-1.4 1.1-2.7.5-2.1-1 2-5 6.5-16.1 4.4-18.9z"/>
                </svg>
            </li>
            <li class="mr-9">
                <svg class="gi gi-digg" width="54.39" height="100%" viewBox="0 170 1122 660" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path d="M0 698V326c0-8.667 4.333-13 13-13h176V184c0-9.333 4.333-14 13-14h98v514c0 9.333-4 14-12 14H0zm109-83h67c8.666 0 13-4.667 13-14V395h-67c-8.667 0-13 4.667-13 14v206zm225 83V326c0-8.667 4.333-13 13-13h98v371c0 9.333-4 14-12 14h-99zm0-445v-69c0-9.333 4-14 12-14h99v69c0 9.333-4 14-12 14h-99zm149 445V326c0-8.667 4.333-13 13-13h288v504c0 8.667-4.334 13-13 13H483l1-69c0-9.333 4-14 12-14h176v-49H483zm110-83h67c8 0 12-4.667 12-14V395h-67c-8 0-12 4.667-12 14v206zm229 146c0-9.333 4.333-14 13-14h176v-49H822V326c0-8.667 4.333-13 13-13h287v504c0 8.667-4 13-12 13H822v-69zm109-146h67c8.666 0 13-4.667 13-14V395h-67c-8.667 0-13 4.667-13 14v206z"/>
                </svg>
            </li>
            <li class="mr-9">
                <svg class="gi gi-quora" width="88.72" height="100%" viewBox="0 0 512 142" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path d="M72.899 110.5c-4.91-9.664-10.67-19.425-21.906-19.425-2.148 0-4.293.355-6.262 1.254l-3.817-7.64c4.651-3.99 12.168-7.154 21.829-7.154 15.03 0 22.743 7.24 28.867 16.481 3.635-7.89 5.363-18.545 5.363-31.752 0-32.98-10.314-49.915-34.407-49.915-23.741 0-33.999 16.935-33.999 49.915 0 32.807 10.258 49.567 34 49.567 3.773 0 7.19-.415 10.332-1.331zm5.885 11.51c-5.204 1.395-10.734 2.165-16.218 2.165C30.953 124.175 0 98.95 0 62.265 0 25.23 30.953 0 62.566 0c32.144 0 62.8 25.05 62.8 62.264 0 20.7-9.66 37.522-23.698 48.395 4.536 6.796 9.206 11.31 15.708 11.31 7.096 0 9.958-5.484 10.437-9.785h9.242c.54 5.727-2.325 29.527-28.153 29.527-15.644 0-23.915-9.067-30.118-19.701zm66.906-31.47V48.13c0-4.83-1.785-6.975-7.332-6.975h-5.906v-11.27h38.587v60.12c0 10.135 5.486 14.604 13.84 14.604 6.855 0 13.713-3.033 17.412-9.958V48.13c0-4.831-1.79-6.976-7.334-6.976h-6.262v-11.27h38.948V92.86c0 6.272 2.324 9.131 9.48 9.131h1.254v11.63l-34.831 5.545V107.18h-.717c-6.738 8.225-16.22 13.775-29.76 13.775-15.21 0-27.38-7.69-27.38-30.415zm143.347 19.146c13.719 0 18.931-11.935 19.205-35.966.266-23.506-5.486-34.885-19.205-34.885-11.986 0-19.502 11.385-19.502 34.885 0 24.036 7.393 35.966 19.502 35.966zm0 11.269c-24.81 0-47.181-18.97-47.181-47.235 0-27.735 21.832-46.164 47.18-46.164 26.423 0 47.596 18.783 47.596 46.164 0 28.265-21.173 47.235-47.595 47.235zm48.394-1.79v-11.271h3.758c9.306 0 10.2-2.683 10.2-10.734V48.13c0-4.83-2.505-6.975-8.229-6.975h-5.01v-11.27h35.364l1.79 18.43h.719c3.937-13.246 14.492-20.224 24.39-20.224 8.17 0 14.55 4.654 14.55 14.08 0 6.559-3.154 13.54-11.983 13.54-7.936 0-9.482-5.371-16.046-5.371-5.842 0-10.378 5.55-10.378 13.716V97.16c0 8.05 1.97 10.734 11.097 10.734h5.187v11.271H337.43zm117.73-13.12c11.274 0 15.923-10.675 15.923-21.404v-14.32c-8.228 8.524-27.377 8.824-27.377 24.033 0 7.455 4.418 11.691 11.453 11.691zm16.463-.061c-4.654 8.945-14.2 14.97-27.556 14.97-15.506 0-25.526-8.235-25.526-22.724 0-29.159 40.614-21.41 52.542-40.73V55.41c0-14.848-5.845-17.173-12.286-17.173-18.071 0-9.843 19.433-26.483 19.433-7.99 0-11.093-4.823-11.093-10.25 0-10.975 13.123-19.863 37.758-19.863 23.314 0 37.632 6.443 37.632 29.645v37.035c0 5.724 2.089 8.83 7.099 8.83 2.145 0 3.934-.597 5.304-1.552l2.985 7.27c-2.447 3.761-8.95 10.381-21.297 10.381-10.731 0-17.472-5.01-18.368-13.181h-.711z"/>
                </svg>
            </li>
            <li class="mr-9">
                <svg class="gi gi-forbes" width="95.14" height="100%" viewBox="0 0 111 28" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path d="M109.085,13.7 L108.085,13.97 C107.135,10.73 105.54,9.342 103.29,9.342 C101.71,9.342 100.59,10.382 100.59,12.042 C100.59,13.622 101.89,14.472 105.424,15.899 C108.677,17.095 110.026,18.559 110.026,21.119 C110.026,25.014 106.966,27.457 101.94,27.457 C99.523,27.457 96.785,26.917 95.242,26.3 L95.049,20.888 L95.999,20.695 C97.426,24.398 99.252,25.799 101.629,25.799 C103.686,25.799 104.766,24.372 104.766,22.906 C104.766,21.44 103.93,20.592 100.716,19.436 C97.656,18.433 95.561,17.122 95.561,13.869 C95.561,10.669 98.376,7.955 103.161,7.955 C105.424,7.955 107.481,8.341 108.831,8.958 L109.085,13.7 Z M88.385,15.397 C88.501,12.852 87.985,9.175 85.891,9.175 C83.744,9.175 82.561,12.608 82.523,15.552 L88.385,15.397 Z M86.405,7.941 C91.675,7.941 94.221,11.798 94.26,16.863 L82.396,16.863 C82.356,21.35 84.736,24.743 89.184,24.743 C91.124,24.743 92.244,24.203 93.426,23.355 L93.979,23.972 C92.679,25.746 89.979,27.442 86.379,27.442 C80.594,27.442 76.5829616,23.47 76.5829616,17.864 C76.57,12.799 79.861,7.939 86.405,7.939 M69.307,17.324 C69.191,13.324 67.727,10.138 64.91,10.138 C64.24,10.138 63.727,10.254 62.93,10.524 L63.16,25.938 C63.443,26.054 64.11,26.131 64.587,26.131 C67.325,26.131 69.305,22.261 69.305,17.325 M75.18,17.025 C75.18,23.363 71.18,27.451 64.394,27.451 C61.977,27.451 59.47,27.065 57.298,26.564 C57.491,24.019 57.491,8.257 57.298,5.287 C57.105,3.744 56.668,3.011 55.472,2.857 L54.675,2.78 L54.675,2.073 L63.045,0.53 C63.005,1.83 62.929,5.287 62.929,9.336 C64.395,8.603 65.744,8.179 67.569,8.179 C71.927,8.179 75.169,11.882 75.169,17.024 M53.869,13.582 C51.285,11.885 48.509,12.618 47.955,13.582 C47.711,16.552 47.762,21.116 47.995,23.468 C48.111,25.011 48.548,25.666 49.743,25.782 L51.323,25.936 L51.323,26.939 L39.5,26.939 L39.5,25.936 L40.413,25.782 C41.557,25.666 41.956,25.012 42.033,23.468 C42.316,20.961 42.393,15.703 42.033,12.81 C41.956,11.152 41.557,10.534 40.413,10.38 L39.5,10.264 L39.5,9.57 L47.946,7.95 L47.83,12.27 C49.81,7.796 53.744,7.256 55.878,8.452 L53.869,13.582 Z M26.152,17.722 C26.152,22.44 27.425,26.297 29.996,26.297 C32.773,26.297 33.57,22.517 33.57,17.722 C33.57,12.927 32.336,9.186 29.726,9.186 C26.926,9.186 26.152,12.85 26.152,17.722 M39.548,17.645 C39.548,23.057 35.974,27.445 29.598,27.445 C23.324,27.445 20.162,23.267 20.162,17.79 C20.162,12.416 23.736,7.942 30.112,7.942 C36.386,7.942 39.548,12.146 39.548,17.635 M23.128,8.327 L21.828,8.713 C20.722,4.92 18.768,2.761 15.4,2.761 L11.003,2.761 C10.759,5.281 10.643,9.176 10.759,13.381 L13.689,13.227 C15.669,13.227 16.504,11.8 16.989,9.727 L18.095,9.727 L18.095,18.495 L16.97,18.495 C16.494,16.375 15.67,14.935 13.67,14.818 L10.74,14.778 C10.74,17.902 10.78,20.576 10.984,22.196 C11.177,24.51 11.82,25.564 13.517,25.783 L15.111,25.937 L15.111,26.94 L0.76,26.94 L0.76,25.937 L1.956,25.783 C3.614,25.553 4.296,24.51 4.489,22.196 C4.809,18.416 4.889,10.343 4.489,5.856 C4.296,3.456 3.615,2.501 1.956,2.308 L0.76,2.115 L0.76,1.075 L23.1,1.075 L23.128,8.327 Z"/>
                </svg>
            </li>
            <li>
                <svg class="gi gi-inc" width="69.63" height="100%" viewBox="0.025 0.047 1151.795 396.953" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                    <path d="M1151.711 343.321c-.113 26.934-21.945 48.565-48.86 48.412-26.479-.15-48.151-22.144-48.003-48.711.149-26.677 22.013-48.196 48.878-48.11 26.282.084 48.094 22.09 47.985 48.409z"/>
                    <path d="M948 230l116-12c-11.333-72-53.667-113-127-123-110-15-178 36-196 62s-27 49-29 91 24 80 33 90 50 59 154 59 138-40 155-77c6.667-15.333 10-28.333 10-39H950c-3.333 30-20.333 45-51 45-46 0-52-43-50-78s-1-81 48-83c32.667-1.333 49.667 20.333 51 65z"/>
                    <path d="M724.815 307.936V390.3H517.333v-85.192h39.175v-2.536c-.003-28.951.007-57.903-.019-86.855-.007-8.475-1.887-16.426-7.074-23.328-5.979-7.958-14.27-12.137-23.812-13.919-17.257-3.223-33.407-.669-47.542 10.325-12.894 10.029-19.572 23.598-19.854 39.747-.441 25.23-.112 50.474-.104 75.711 13.427.733 26.719.733 40.17.733v85.333H286.097v-85.182h38.157V177.15h-38.17V91.853H453.48v58.814c2.492-2.669 4.481-4.625 6.273-6.748 12.999-15.398 28.181-28.003 46.554-36.543 12.787-5.943 26.241-9.559 40.208-11.25 23.628-2.86 47.13-2.331 70.365 3.2 16.984 4.043 32.591 10.967 45.703 22.826 13.632 12.328 21.818 27.768 25.159 45.651a140.345 140.345 0 0 1 2.383 24.652c.25 36.378.108 72.759.109 109.14v3.303h34.581zM50.669 91.721H.025V.047h271.513v91.148h-50.66v213.727h50.628v85.349H.074v-85.149h50.595V91.721z"/>
                </svg>
            </li>
        </ul>
    </div>
</div>
<?=$this->section('content')?>
<footer class="card pt-11 rounded-0">
    <div class="container">
        <div class="row">
            <div class="col-sm-6 col-lg-3">
                <h2 class="mb-5 fs-xxs">Products</h2>
                <ul class="list-unstyled mb-5">
                    <li class="mb-3">
                        <a class="link-body" href="#">Droplets</a>
                    </li>
                    <li class="mb-3">
                        <a class="link-body" href="#">Spaces</a>
                    </li>
                    <li class="mb-3">
                        <a class="link-body" href="#">Kubernetes</a>
                    </li>
                    <li>
                        <a class="link-body" href="#">Block Storage</a>
                    </li>
                </ul>
            </div>
            <div class="col-sm-6 col-lg-3">
                <h2 class="mb-5 fs-xxs">Resources</h2>
                <ul class="list-unstyled mb-5">
                    <li class="mb-3">
                        <a class="link-body" href="#">FAQ</a>
                    </li>
                    <li class="mb-3">
                        <a class="link-body" href="#">Blog</a>
                    </li>
                    <li>
                        <a class="link-body" href="#">Docs</a>
                    </li>
                </ul>
            </div>
            <div class="col-sm-6 col-lg-3">
                <h2 class="mb-5 fs-xxs">Company</h2>
                <ul class="list-unstyled mb-5">
                    <li class="mb-3">
                        <a class="link-body" href="#">About</a>
                    </li>
                    <li class="mb-3">
                        <a class="link-body" href="#">Careers</a>
                    </li>
                    <li>
                        <a class="link-body" href="#">Contacts</a>
                    </li>
                </ul>
            </div>
            <div class="col-sm-6 col-lg-3 mb-11">
                <a class="link-body" href="#">
                    <svg class="gi gi-github" width="88.73" height="24" viewBox="-0.122 175.062 480.244 129.875" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                        <path d="M92.375 230.65H52.019a1.887 1.887 0 0 0-1.886 1.886v19.731a1.89 1.89 0 0 0 1.886 1.889h15.743v24.513s-3.535 1.206-13.308 1.206c-11.53 0-27.637-4.214-27.637-39.632 0-35.425 16.772-40.087 32.518-40.087 13.63 0 19.502 2.401 23.238 3.556 1.174.36 2.26-.808 2.26-1.851l4.502-19.063c0-.487-.165-1.075-.721-1.474-1.517-1.082-10.774-6.262-34.16-6.262-26.941 0-54.576 11.463-54.576 66.562 0 55.103 31.64 63.313 58.302 63.313 22.076 0 35.468-9.434 35.468-9.434.552-.306.612-1.077.612-1.431v-61.537c0-1.04-.844-1.885-1.885-1.885M300.354 181.664a1.88 1.88 0 0 0-1.873-1.896h-22.724a1.889 1.889 0 0 0-1.882 1.896l.006 43.913h-35.419v-43.913a1.882 1.882 0 0 0-1.875-1.896h-22.722a1.887 1.887 0 0 0-1.877 1.896v118.904c0 1.048.843 1.901 1.877 1.901h22.722c1.039 0 1.875-.854 1.875-1.901v-50.86h35.419l-.062 50.86c0 1.048.844 1.901 1.883 1.901h22.777c1.041 0 1.873-.854 1.875-1.901V181.664M135.254 197.268c0-8.183-6.56-14.795-14.653-14.795-8.085 0-14.65 6.612-14.65 14.795 0 8.173 6.565 14.802 14.65 14.802 8.093 0 14.653-6.629 14.653-14.802M133.629 275.486V220.6a1.89 1.89 0 0 0-1.88-1.895h-22.651c-1.039 0-1.969 1.072-1.969 2.115v78.634c0 2.312 1.44 2.998 3.304 2.998h20.408c2.239 0 2.788-1.1 2.788-3.034v-23.932M386.71 218.884h-22.549c-1.034 0-1.876.852-1.876 1.901v58.302s-5.729 4.192-13.859 4.192c-8.13 0-10.287-3.689-10.287-11.65v-50.844c0-1.049-.841-1.901-1.875-1.901h-22.886c-1.032 0-1.879.852-1.879 1.901v54.693c0 23.646 13.179 29.431 31.31 29.431 14.873 0 26.864-8.216 26.864-8.216s.571 4.329.829 4.843c.259.512.932 1.029 1.659 1.029l14.56-.064c1.032 0 1.879-.854 1.879-1.898l-.008-79.817c0-1.05-.843-1.902-1.882-1.902M439.445 283.207c-7.821-.238-13.126-3.788-13.126-3.788v-37.653s5.233-3.208 11.654-3.782c8.12-.727 15.944 1.726 15.944 21.095.001 20.426-3.53 24.457-14.472 24.128zm8.894-66.993c-12.807 0-21.518 5.714-21.518 5.714v-40.263c0-1.05-.839-1.897-1.875-1.897h-22.787a1.887 1.887 0 0 0-1.878 1.897v118.903c0 1.048.842 1.901 1.881 1.901h15.811c.711 0 1.25-.367 1.648-1.009.393-.639.96-5.482.96-5.482s9.318 8.83 26.957 8.83c20.708 0 32.584-10.504 32.584-47.154 0-36.651-18.967-41.44-31.783-41.44M198.936 218.695h-17.045l-.026-22.519c0-.852-.439-1.278-1.424-1.278h-23.228c-.903 0-1.388.398-1.388 1.265v23.271s-11.64 2.809-12.427 3.037a1.886 1.886 0 0 0-1.361 1.812v14.623c0 1.052.84 1.898 1.879 1.898h11.909v35.178c0 26.13 18.328 28.697 30.696 28.697 5.651 0 12.411-1.814 13.527-2.228.675-.248 1.067-.947 1.067-1.705l.019-16.086c0-1.05-.886-1.897-1.884-1.897-.993 0-3.534.404-6.15.404-8.372 0-11.209-3.893-11.209-8.932l-.001-33.432h17.046c1.039 0 1.88-.847 1.88-1.898v-18.317a1.884 1.884 0 0 0-1.88-1.893"/>
                    </svg>
                </a>
                <p class="my-5">The quick brown fox jumps over the lazy dog.</p>
                <ul class="list-unstyled d-flex">
                    <li class="mr-3">
                        <a class="btn btn-circle btn-neutral" href="#">
                            <svg class="gi gi-twitter-fill fs-sm" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M8.08 20A11.07 11.07 0 0 0 19.52 9 8.09 8.09 0 0 0 21 6.16a.44.44 0 0 0-.62-.51 1.88 1.88 0 0 1-2.16-.38 3.89 3.89 0 0 0-5.58-.17A4.13 4.13 0 0 0 11.49 9C8.14 9.2 5.84 7.61 4 5.43a.43.43 0 0 0-.75.24 9.68 9.68 0 0 0 4.6 10.05A6.73 6.73 0 0 1 3.38 18a.45.45 0 0 0-.14.84A11 11 0 0 0 8.08 20"/>
                            </svg>
                        </a>
                    </li>
                    <li class="mr-3">
                        <a class="btn btn-circle btn-neutral" href="#">
                            <svg class="gi gi-linkedin-fill fs-sm" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M15.15 8.4a5.83 5.83 0 0 0-5.85 5.82v5.88a.9.9 0 0 0 .9.9h2.1a.9.9 0 0 0 .9-.9v-5.88a1.94 1.94 0 0 1 2.15-1.93 2 2 0 0 1 1.75 2v5.81a.9.9 0 0 0 .9.9h2.1a.9.9 0 0 0 .9-.9v-5.88a5.83 5.83 0 0 0-5.85-5.82z"/>
                                <rect x="3" y="9.3" width="4.5" height="11.7" rx=".9" ry=".9"/>
                                <circle cx="5.25" cy="5.25" r="2.25"/>
                            </svg>
                        </a>
                    </li>
                    <li>
                        <a class="btn btn-circle btn-neutral" href="#">
                            <svg class="gi gi-facebook-fill fs-sm" width="1em" height="1em" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path d="M17 3.5a.5.5 0 0 0-.5-.5H14a4.77 4.77 0 0 0-5 4.5v2.7H6.5a.5.5 0 0 0-.5.5v2.6a.5.5 0 0 0 .5.5H9v6.7a.5.5 0 0 0 .5.5h3a.5.5 0 0 0 .5-.5v-6.7h2.62a.5.5 0 0 0 .49-.37l.72-2.6a.5.5 0 0 0-.48-.63H13V7.5a1 1 0 0 1 1-.9h2.5a.5.5 0 0 0 .5-.5z"/>
                            </svg>
                        </a>
                    </li>
                </ul>
            </div>
        </div>
        <p class="text-center">The quick brown fox jumps over the lazy dog</p>
        <hr class="mb-0">
        <div class="py-6">
            <div class="row">
                <div class="col-md mb-5 mb-md-0">
                    <p class="text-center text-md-left">© 2020 Brights</p>
                </div>
                <div class="col-md">
                    <ul class="list-unstyled d-flex flex-wrap justify-content-center justify-content-md-end">
                        <li>
                            <a class="link-body" href="#">Terms of Service</a>
                        </li>
                        <li class="ml-5">
                            <a class="link-body" href="#">Privacy Policy</a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</footer>
<script src="https://code.jquery.com/jquery-3.4.1.slim.min.js" integrity="sha384-J6qa4849blE2+poT4WnyKhv5vZF5SrPo0iEjwBvKU7imGFAV0wwj1yYfoRSJoZ+n" crossorigin="anonymous"></script>
<script src="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/js/bootstrap.bundle.min.js" integrity="sha384-6khuMg9gaYr5AxOqhkVIODVIvm9ynTT5J4V1cfthmT+emCG6yVmEZsRHdxlotUnm" crossorigin="anonymous"></script>
</body>
</html>
